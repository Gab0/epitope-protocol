...
def ExtractStructurallyRelevantMutations(ProteinAlignment, PDBSequence):
...
def sort_unique_sequences(sequences, reference_sequence) -> List[str]:
    ...
    while also starting with the refeoence sequence.
...
def RunStructureAgainstSequences(
    ...
    for idx, output_file in enumerate(OutputFiles):
    ...
    try:
        Matrix = buildRMSDMatrix(
            [f.output_filepath for f in OutputFiles],
            ModelVersions)
        print(Matrix)
...
def parse_arguments(description=__doc__):
